...
public class CardBartok : Card {
	...
	override public void OnMouseUpAsButton() {
		...
		Bartok bartok = GameObject.Find ("Main Camera").GetComponent<Bartok> ();
		if (this.state == CBState.drawpile && bartok.gameState == BartokGameState.Player1) {
			bartok.giveCardToPlayer (0);
			bartok.gameState = BartokGameState.Player2;
		} else if (this.state == CBState.hand && bartok.gameState == BartokGameState.Player1) {
			if (bartok.tryCard (this, 0)) {
				bartok.gameState = BartokGameState.Player2;
			}
		}
	}
